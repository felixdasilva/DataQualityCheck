...
with st.sidebar.beta_expander("5. Specify Upper / Lower bounds", expanded=False):
    ...
    accuracy_selected = pd.DataFrame(np.array([[1],[2]]))
...
#Display Objective
    ...
    UpperViolation = (df[df.accuracy_selected] > Upper_Bound).sum()
    LowerViolation = (df[df.accuracy_selected] < Lower_Bound).sum()
