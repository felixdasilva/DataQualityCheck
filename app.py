...
with st.sidebar.beta_expander("4. Select Validation Rules", expanded=False):
    st.write('Select columns to check for email format')
...
#Validity
    ...
    df['isemail'] = df2['email'].apply(lambda x: True if pattern.match(x) else False)
    st.write(df2)
